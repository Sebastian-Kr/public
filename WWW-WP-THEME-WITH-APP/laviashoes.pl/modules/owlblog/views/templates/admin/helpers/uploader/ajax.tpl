...
<div class="form-group" style="display: none;">
	<div class="col-lg-12" id="{$id|intval}-images-thumbnails">
		...
		<div class="img-thumbnail text-center">
			<p>{$file.image nofilter}</p>
			{if isset($file.size)}<p>{l s='File size' mod='owlblog'} {$file.size|escape:'html':'UTF-8'}kb</p>{/if}
			{if isset($file.delete_url)}
			<p>
				<a class="btn btn-default" href="{$file.delete_url nofilter}">
					<i class="icon-trash"></i> {l s='Delete' mod='owlblog'}
				</a>
			</p>
			{/if}
		</div>
...
<div class="row">
	<div class="alert alert-warning">{l s='You have reached the limit (%s) of files to upload, please remove files to continue uploading' sprintf=$max_files mod='owlblog'}</div>
</div>
...
<div class="form-group">
	<div class="col-lg-12">
		...
		<button class="btn btn-default" data-style="expand-right" data-size="s" type="button" id="{$id|intval}-add-button">
			<i class="icon-plus-sign"></i> {if isset($multiple) && $multiple}{l s='Add files' mod='owlblog'}{else}{l s='Add file' mod='owlblog'}{/if}
		</button>
		<button class="ladda-button btn btn-default" data-style="expand-right" type="button" id="{$id|intval}-upload-button" style="display:none;">
			<i class="icon-cloud-upload text-success"></i> <span class="ladda-label text-success">{if isset($multiple) && $multiple}{l s='Upload files' mod='owlblog'}{else}{l s='Upload file' mod='owlblog'}{/if}</span>
		</button>
	</div>
</div>
...
<script type="text/javascript">
	...
	$( document ).ready(function() {
		...
		$('#{$id|intval}').fileupload({
			...
			{if isset($post_max_size)}maxFileSize: {$post_max_size nofilter},{/if}
			{if isset($drop_zone)}dropZone: {$drop_zone nofilter},{/if}
			start: function (e) {
								...
								else 
								{
									...
									if (typeof data.result.{$name|escape:'html':'UTF-8'}[i].image !== 'undefined')
									{
										...
										if (typeof data.result.{$name|escape:'html':'UTF-8'}[i].delete_url !== 'undefined')
											template += '<p><a class="btn btn-default" href="'+data.result.{$name|escape:'html':'UTF-8'}[i].delete_url+'"><i class="icon-trash"></i> {l s='Delete' mod='owlblog'}</a></p>';
			...
			if (typeof {$id|intval}_max_files !== 'undefined') {
				if ({$id|intval}_total_files >= {$id|intval}_max_files) {
					e.preventDefault();
					alert('{l s='You can upload a maximum of %s files' mod='owlblog'}'+$max_files);
					return;
				}
			}
			...
			var button = $('<button/>').addClass('btn btn-default pull-right').prop('type', 'button').html('<i class="icon-trash"></i> {l s='Remove file' mod='owlblog'}').appendTo(data.context).on('click', function() {
				{$id|intval}_total_files--;
				data.files = null;
