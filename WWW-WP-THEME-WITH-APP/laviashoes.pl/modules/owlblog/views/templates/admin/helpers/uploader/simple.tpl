		...
		<div class="dummyfile input-group">
			...
			<span class="input-group-btn">
				<button id="{$id|intval}-selectbutton" type="button" name="submitAddAttachments" class="btn btn-default">
					<i class="icon-folder-open"></i> {if isset($multiple) && $multiple}{l s='Add files' mod='owlblog'}{else}{l s='Add file' mod='owlblog'}{/if}
				</button>
		...
		if (typeof {$id|escape:'html':'UTF-8'}_max_files !== 'undefined')
		{
			$('#{$id|intval}').closest('form').on('submit', function(e) {
				if ($('#{$id|intval}')[0].files.length > {$id|escape}_max_files) {
					e.preventDefault();
					alert("{l s='You can upload a maximum of %s files'|sprintf:$max_files  mod='owlblog'}");
				}
			});
		}
	});
