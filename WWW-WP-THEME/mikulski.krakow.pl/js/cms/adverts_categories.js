...
function selectTabByHash(){
    ...
    if(x){
        x = x.substring(1);
        x = x.replace('Link','');
        if($('ul.adverts_categories-nav li').find('.'+x)){
            $('ul.adverts_categories-nav li').find('.'+x).click();
        }
    }
}
...
function initEditStatus(){    
    ...
    $('#table-sorting a.categoryStatus, #table-subcat-sorting a.categoryStatus').editable({
        ...
        name: 'adverts_categoriesStatus',
        url: '/cms/adverts/ajax/setStatus',
        ...
        success: function(data) {
            if(data == 1){
                ...
                showNotification("Zmieniono aktywność rekomendacji", "Rekomendacje", "bottom-right");
            } else {
...
function initSubCatSearch(){    
    $("#subadverts_categoriesSearch").keyup(function() {
        if ($(this).val() != "")
        {
...
$(function() {   
    ...
    $("#adverts_categoriesSearch").parent().parent().unbind().submit(function (e) {
        return false;
    });
    ...
    $("#adverts_categoriesSearch").keyup(function() {
        if ($(this).val() != "")
        {
            sortableDisable();
        ...
        $("#table-sorting tbody").sortable({
            ...
            stop: function(event, ui) {
                ...
                if (iStartPosition != iNewPosition) {
                    $.ajax({
                        url: "/cms/adverts/ajax/setNewOrder",
                        type: "POST",
                        data: {
                            'adverts_categories-id' : iId,
                            'old-position' : iStartPosition,
                            'new-position' : iNewPosition
                        },
                        success: function(data) {
                            if(data == 1){
                                showNotification("Zapisano zmianę kolejności", "Rekomendacje", "bottom-right");
                            } else {
                                alert('Wystąpił nieoczekiwany błąd');
                                location.reload();
                            }
            ...
            stop: function(event, ui) {
                ...
                if (iStartPosition != iNewPosition) {
                    $.ajax({
                        ...
                        success: function(data) {
                            if(data == 1){
                                showNotification("Zapisano zmianę kolejności", "Rekomendacje - podkategorie", "bottom-right");
                            } else {
                                alert('Wystąpił nieoczekiwany błąd');
                                location.reload();
                            }
...
function deleteCatForm(oAnchor, id) { 
    ...
    $.ajax({
        url: "/cms/adverts/ajax/deleteCat",
        type: "POST",
        data: {
            'adverts_categories-id' : id
        },
        success: function(data) {
            if(data == 1){                
                ...
                showNotification("Usunięto Kategorię", "Rekomendacje", "bottom-right");
...
function saveNewSubCategoryWindow(oAnchor){
    ...
    if($('#addSubCat form').parsley().isValid()){
        ...
        $.ajax({
            url: "/cms/adverts/ajax/addNewSubcategory",
            type: "POST",
            data: {
            ...
            success: function(data) {
                if(data == 1){
                    showNotification("Dodano podkategorię", "Rekomendacje - podkategorie", "bottom-right");
...
function refreshSubcategoryTab(iId) {
    $.ajax({
        url: "/cms/adverts/ajax/refresh-subcat",
        type: "POST",
        data: {
            'adverts_categories-id': iId
        },
        success: function(data) {
            $('#TabSubadverts_categories').html(data);
            initTableSubCatSort();
            initEditStatus();
            initSubCatSearch();
        }
...
function deleteSubCat(iId) {    
    ...
    $.ajax({
        ...
        success: function(data) {
                ...
                showNotification("Usunięto kategorię", "Rekomendacje - podkategorie", "bottom-right");
