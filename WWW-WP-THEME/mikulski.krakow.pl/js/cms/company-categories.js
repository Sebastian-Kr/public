...
function selectTabByHash(){
    ...
    if(x){
        x = x.substring(1);
        x = x.replace('Link','');
        if($('ul.company_categories-nav li').find('.'+x)){
            $('ul.company_categories-nav li').find('.'+x).click();
        }
    }
}
...
function initEditStatus(){    
    ...
    $('#table-sorting a.categoryStatus, #table-subcat-sorting a.categoryStatus').editable({
        ...
        name: 'company_categoriesStatus',
        url: '/cms/company/ajax/setStatus',
    ...
    $('#table-sorting a.categoryBg, #table-subcat-sorting a.categoryBg').editable({
        type: 'select',
        source: [{value: '1', text: 'tak'}, {value: '0', text: 'nie'}],
        name: 'company_categoriesBg',
        url: '/cms/company/ajax/setBg',
        title: 'Wybierz status',
        send:'always',
        success: function(data) {
            if(data == 1){
                ...
                selectVal = parseInt(selectVal)>0?'active':'';
                $(label).removeClass().addClass('label' + ' ' + selectVal); // remove actual class from label and add new class defined by selected value (this change color of label)
                showNotification("Zmieniono widoczność tła", "Kategorie", "bottom-right");
            } else {
                alert('Wystąpił nieoczekiwany błąd');
                location.reload();
            }
...
function initSubCatSearch(){    
    $("#subcompany_categoriesSearch").keyup(function() {
        if ($(this).val() != "")
        {
...
$(function() {   
    ...
    $("#company_categoriesSearch").parent().parent().unbind().submit(function (e) {
        return false;
    });
    ...
    $("#company_categoriesSearch").keyup(function() {
        if ($(this).val() != "")
        {
            sortableDisable();
        ...
        $("#table-sorting tbody").sortable({
            ...
            stop: function(event, ui) {
                ...
                if (iStartPosition != iNewPosition) {
                    $.ajax({
                        url: "/cms/company/ajax/setNewOrder",
                        type: "POST",
                        data: {
                            'company_categories-id' : iId,
...
function deleteCatForm(oAnchor, id) { 
    ...
    $.ajax({
        url: "/cms/company/ajax/deleteCat",
        type: "POST",
        data: {
            'company_categories-id' : id
        },
        success: function(data) {
            if(data == 1){                
...
function saveNewSubCategoryWindow(oAnchor){
    ...
    if($('#addSubCat form').parsley().isValid()){
        ...
        $.ajax({
            url: "/cms/company/ajax/addNewSubcategory",
            type: "POST",
            data: {
...
function refreshSubcategoryTab(iId) {
    $.ajax({
        url: "/cms/company/ajax/refresh-subcat",
        type: "POST",
        data: {
            'company_categories-id': iId
        },
        success: function(data) {
            $('#TabSubcompany_categories').html(data);
            initTableSubCatSort();
            initEditStatus();
            initSubCatSearch();
        }
...
function deleteSiteCategoryPhoto(iId) {
    $('#windows-ask-confirm').modal('hide');
    $.ajax({
        url: "/cms/company/ajax/delete-category-photo",
        type: "POST",
        data: {
            'company-cat-id': iId
        },
        success: function (data) {
            ...
            showNotification("Tło zostało usunięta", "Kategorie - tło", "bottom-right");
        }
    });
}