...
function selectTabByHash() {
    ...
    if (x) {
        x = x.substring(1);
        x = x.replace('Link', '');
        if ($('ul.partners-nav li').find('.' + x)) {
            $('ul.partners-nav li').find('.' + x).click();
        }
    }
}
...
function tableGridSearch(lang_code) {
    $("#partnersSearch" + lang_code).keyup(function () {
        if ($(this).val() != "")
        {
    ...
    $("#partnersSearch" + lang_code).parent().parent().unbind().submit(function (e) {
        return false;
    });
}
...
function tableGridEdit(lang_code) {
    $('#table-sorting-' + lang_code + ' a.newsStatus').editable({
        ...
        name: 'partnersStatus',
        url: '/cms/partners/ajax/setStatus',
        ...
        success: function (data) {
            ...
            if (data.type == 1) {
                ...
                showNotification("Zmieniono aktywność partnera", "Partnerzy", "bottom-right");
            } else if (data.type == 2) {
       ...
       $('#table-sorting-' + lang_code + ' a.newsMan').editable({
        type: 'select',
        source: [{value: '1', text: 'wieloletni'}, {value: '0', text: 'korporacyjny'}],
        name: 'partnersMan',
        url: '/cms/partners/ajax/setMan',
        title: 'Wybierz typ',
        send: 'always',
        success: function (data) {
            var data = $.parseJSON(data);
            if (data.type == 1) {
                ...
                selectVal = parseInt(selectVal) > 0 ? 'active' : '';
                $(label).removeClass().addClass('label' + ' ' + selectVal); // remove actual class from label and add new class defined by selected value (this change color of label)
                showNotification("Zmieniono typ partnera", "Partnerzy", "bottom-right");
            } else if (data.type == 2) {
                showNotification(data.msg, "Error", "bottom-right");
                alert(data.msg);
                setTimeout(function () {
...
$(function () {
    ...
    selectTabByHash();
    $('#addPartnerLangSelect').multiselect();
    $('#addPartner form').parsley({
    ...
    if (parseInt($("#table-sorting-" + lang_code + " tbody tr").length) > 1) {
        ...
        $("#table-sorting-" + lang_code + " tbody").sortable({
            ...
            stop: function (event, ui) {
                ...
                var iId = ui.item.attr('id').replace('partners-list-item-', '');
                if (iStartPosition != iNewPosition) {
                    $.ajax({
                        url: "/cms/partners/ajax/setNewOrder",
                        type: "POST",
                        data: {
                            'partners-id': iId,
                        ...
                        success: function (data) {
                            if (data == 1) {
                                showNotification("Zapisano zmianę kolejności", "Partnerzy", "bottom-right");
                            } else {
                                alert('Wystąpił nieoczekiwany błąd');
                                location.reload();
                            }
...
function deletePartnerForm(oAnchor, id, lang_code) {
    var $ = jQuery;
    var sActionButton = $(oAnchor).attr('onclick');
    $(oAnchor).attr('onclick', '');
    $.ajax({
        url: "/cms/partners/ajax/deleteSite",
        type: "POST",
        data: {
            'partners-id': id
        },
        success: function (data) {
            if (data == 1) {
                $("#partners-list-item-" + id).fadeOut("slow", function () {
                    $(this).remove();
                });
                $('#windows-ask-confirm').modal('hide');
                showNotification("Usunięto partnera", "Partnerzy", "bottom-right");
...
function deleteSitePhoto(iId) {
    $('#windows-ask-confirm').modal('hide');
    $.ajax({
        url: "/cms/partners/ajax/delete-avatar",
        type: "POST",
        data: {
            'partners-id': iId
        },
        success: function (data) {
            $('#gallery-list-item-' + iId).fadeOut("slow", function () {
                $(this).remove();
                $(".imagesContainer").parent().append('<h4>Brak logotypu</h4>');
                $(".imagesContainer").remove();
            });
            showNotification("Logotyp został usunięty", "Partnerzy - logo", "bottom-right");
        }
    });
}