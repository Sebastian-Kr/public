...
function selectTabByHash(){
    ...
    if(x){
        x = x.substring(1);
        x = x.replace('Link','');
        if($('ul.ecards-nav li').find('.'+x)){
            $('ul.ecards-nav li').find('.'+x).click();
        }
    }
}
...
function tableGridSearch(lang_code){
      $("#ecardsSearch"+lang_code).keyup(function() {
        if ($(this).val() != "")
        {
    ...
    $('#table-sorting-'+lang_code+' a.newsStatus').editable({
        ...
        success: function(data) {
            ...
            if(data.type == 1){
                ...
                showNotification("Zmieniono aktywność", "E-kartki", "bottom-right");
            } else if(data.type == 2) {
...
$(function() {   
    ...
    selectTabByHash();
    $('#addEcardsLangSelect').multiselect();
    $('#addEcards form').parsley({
    ...
    if (parseInt($("#table-sorting-"+lang_code+" tbody tr").length) > 1) {
        ...
        $("#table-sorting-"+lang_code+" tbody").sortable({
            ...
            stop: function(event, ui) {
                ...
                var iId = ui.item.attr('id').replace('ecards-list-item-', '');
                if (iStartPosition != iNewPosition) {
                    $.ajax({
                        url: "/cms/ecards/ajax/setNewOrder",
                        type: "POST",
                        data: {
                            'ecards-id' : iId,
                        ...
                        success: function(data) {
                            if(data == 1){
                                showNotification("Zapisano zmianę kolejności", "E-kartki", "bottom-right");
                            } else {
                                alert('Wystąpił nieoczekiwany błąd');
                                location.reload();
                            }
...
function deleteEcardsForm(oAnchor, id, lang_code) { 
    var $ = jQuery;
    var sActionButton = $(oAnchor).attr('onclick');
    $(oAnchor).attr('onclick','');
    $.ajax({
        url: "/cms/ecards/ajax/deleteSite",
        type: "POST",
        data: {
            'ecards-id' : id
        },
        success: function(data) {
            if(data == 1){                
                $("#ecards-list-item-"+id).fadeOut("slow", function() {
                    $(this).remove();
                });
                $('#windows-ask-confirm').modal('hide'); 
                showNotification("Usunięto e-kartkę", "E-kartki", "bottom-right");
...
function deleteSitePhoto(iId) {    
    $('#windows-ask-confirm').modal('hide');    
    $.ajax({
        url: "/cms/ecards/ajax/delete-avatar",
        type: "POST",
        data: {
            'ecards-id' : iId
        },
        success: function(data) {
            ...
            showNotification("E-kartka została usunięta", "E-kartki", "bottom-right");            
        }
    });
}