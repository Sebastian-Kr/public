...
function selectTabByHash(){
    ...
    if(x){
        x = x.substring(1);
        if($('ul.categories-nav li').find('.'+x)){
            $('ul.categories-nav li').find('.'+x).click();
        }
    }
}
...
function initEditStatus(){    
    ...
    $('#table-sorting a.categoryStatus, #table-subcat-sorting a.categoryStatus').editable({
        ...
        url: '/cms/news-categories/ajax/setStatus',
...
function initSubCatSearch(){    
    $("#subcategoriesSearch").keyup(function() {
        if ($(this).val() != "")
        {
        ...
        $("#table-sorting tbody").sortable({
            ...
            stop: function(event, ui) {
                ...
                if (iStartPosition != iNewPosition) {
                    $.ajax({
                        url: "/cms/news-categories/ajax/setNewOrder",
                        type: "POST",
                        data: {
                            'categories-id' : iId,
...
function deleteCatForm(oAnchor, id) { 
    ...
    $.ajax({
        url: "/cms/news-categories/ajax/deleteCat",
        type: "POST",
        data: {
            'categories-id' : id
        },
...
function saveNewSubCategoryWindow(oAnchor){
    ...
    if($('#addSubCat form').parsley().isValid()){
        ...
        $.ajax({
            url: "/cms/news-categories/ajax/addNewSubcategory",
            type: "POST",
            data: {
...
function refreshSubcategoryTab(iId) {
    $.ajax({
        url: "/cms/news-categories/ajax/refresh-subcat",
        type: "POST",
        data: {
            'categories-id': iId
        },
        success: function(data) {
            $('#TabSubcategories').html(data);
            initTableSubCatSort();
            initEditStatus();
            initSubCatSearch();
        }
