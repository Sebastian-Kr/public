...
(function() {
  ...
  function moveIt()
  {
      for(i = 0; i < circs.length; ++i)
      {
          ...
          circs[i].attr({cx: nowX, cy: nowY});
              ...
              // Calc growth
          rad = circs[i].attr("r");
          if (circs[i].grow > 0) circs[i].attr("r", Math.min(130, rad +  .1));
          ...
          if (circs[i].fade > 0) {
              ...
              circs[i].attr("fill-opacity", Math.min(0.4, opa +  .01));
              circs[i].attr("stroke-opacity", Math.min(0.4, opa +  .01)); }
  ...
  window.onload = function () {
      paper = Raphael("canvas", 2000, 500);
      circs = paper.set();
      for (i = 0; i < 120; ++i)
      {
          opa = ran(3,10)/10;
          // size circe set !!
          circs.push(paper.circle(ran(0,2500), ran(0,600), ran(60,130)).attr({"fill-opacity": opa,
                                                                         "stroke-opacity": opa}));
      }
      circs.attr({fill: "#fff", stroke: "#fff"});
      moveIt();
