...
function progremo_setup() {
  ...
  add_image_size( 'xxxl', 2400, 9999 ); // 2400 unlimited
	...
	function prgrm_srcSet($acf_field, $acf_field_mobile = NULL) {
		...
		// $urlMD = wp_get_attachment_image_src(get_field($acf_field_mobile), 'md' );
		// $urlLG = wp_get_attachment_image_src(get_field($acf_field), 'lg' );
		// $urlXL = wp_get_attachment_image_src(get_field($acf_field), 'xl' );
		// $urlXXXL = wp_get_attachment_image_src(get_field($acf_field), 'xxxl' );

		// echo ' data-smsrc="'.$urlSM[0].'"; ';
		// echo ' data-mdsrc="'.$urlMD[0].'"; ';
...
add_filter('use_block_editor_for_post', '__return_false');
 ...
 // NEW POST TYPES
  function create_post_type_destination() {
    register_post_type( 'destination',
      array(
        'labels' => array(
					'name' => 'Destynacje',
    'singular_name' => 'Destynacja',
    'add_new' => 'Dodaj nowa destynacja',
    'add_new_item' => __('Dodaj nową destynację'),
    'edit_item' => __('Edytuj destnację'),
    'new_item' => __('Nowa destynacja'),
    'all_items' => __('Wszystkie destynacja'),
    'view_item' => __('Zobacz destynację'),
    'search_items' => __('Wyszukaj destynację'),
    'not_found' =>  __('Żadnen wynik nie został znaleziony'),
    'not_found_in_trash' => __('Żaden wynik nie został znaleziony w koszu'),
    'parent_item_colon' => '',
    'menu_name' => __('Destynacje')
        ),
     'public' => true,
  ...
  add_action( 'init', 'create_post_type_destination' );
