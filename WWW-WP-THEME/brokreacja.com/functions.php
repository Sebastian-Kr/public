...
function generate_code($len = 100, $code_type = 'numbers')
{
        $pattern_numbers = "0123456789";
        $pattern_chars = "QWERTYUIOPASDFGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm";
        $pattern_mixed = $pattern_numbers . $pattern_chars;

        $random_code = "";


        if($code_type == 'numbers') { $pattern = $pattern_numbers; }
        else if($code_type == 'chars') { $pattern = $pattern_chars; }
        else { $pattern = $pattern_mixed; }


        $rand_len = strlen($pattern) - 1;


        for($i=1; $i<=$len; $i++)
        {
            $random_code .= $pattern{mt_rand(0,$rand_len)};
        }
...
function parse_attached_file($file)
{
...
function create_post_type() {
  register_post_type( 'podstawowa',
    array(
      'labels' => array(
        'name' => __( 'Seria podstawowa' ),
        'singular_name' => __( 'podstawowa' )
      ),
      'has_archive' => true,
      'public' => true,
...
function create_post_type2() {
  register_post_type( 'heavy',
    array(
      'labels' => array(
        'name' => __( 'Seria heavy' ),
        'singular_name' => __( 'heavy')
      ),
      'has_archive' => true,
      'public' => true,
    'capability_type' => 'post',
